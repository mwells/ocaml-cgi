...
struct
  ...
  let of_stream stream =
    ...
    match Scgi_headers.of_string decoded with
          ...
          match scgi with
            | "1"  ->
              ...
              Lwt.return @< make
                ...
                uri
                headers
                (lazy (lwt chars = Lwt_stream.nget content_length stream in
    ...
    Lwt_io.establish_server
      ...
      (fun (inch, ouch) ->
        Lwt.ignore_result (
          lwt response =
            try_lwt
              lwt request = Scgi_request.of_stream (Lwt_io.read_chars inch) in
              f request
            with e ->
              (* TODO log it *)
