...
let tests =
  ["cookies", (fun () ->
    ...
    assert_equal ~printer ~msg:"123456" ~expected:(Some "123456") (Request.cookie r "uid") >>= fun () ->
    let r = make "uid=ABCDEFG; foo=bar; baz=biz" in
    assert_equal ~printer ~msg:"ABCDEFG" ~expected:(Some "ABCDEFG") (Request.cookie r "uid") >>= fun () ->
    assert_equal ~printer ~msg:"foo=bar" ~expected:(Some "bar") (Request.cookie r "foo") >>= fun () ->
